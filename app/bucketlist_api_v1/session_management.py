...
def register():
    ...
    if not username or not email_add or not password:
        return jsonify({
            "message": "Enter the username, email address and password" +
            " to create account"}), 400
    if not validators.email(email_add):
        return jsonify({
            "message": "Email address should have 'email@example.com' format"
        }), 400
    if username.isdigit() or username.isspace() or password.isspace():
        return jsonify({
            "message": "Invalid value entered for username and / or password"
        })
    ...
    return jsonify({
        "Successful Added":
        [{'username': user.username, 'email address': user.email_add}]
    }), 201
...
def login():
    ...
    return jsonify({'Generated Token': user.generate_token()}), 200
